...
const newArray = [];
  for (let i = 0; i < array.length; i++) {
    ...
    if (parseInt(yearsArray[0]) >= 1900 && parseInt(yearsArray[1]) <= 2000) {
...
for (let i = 0; i < array.length; i++) {
  let srcArr = array[i].name.split(' ');
  let src = srcArr.join('-');
  if (srcArr.length === 1) {
    src = srcArr[0];
  }
  let wikiArr = array[i].name.split(' ');
  let wiki = wikiArr.join('_');
  if (wikiArr.length === 1) {
    wiki = wikiArr[0];
  }
  console.log(`<div id="artist">\n<div class="image">\n<img src="${src}"/>\n</div>\n<div class = "name">\n<a href="https://en.wikipedia.org/wiki/${wiki}"> ${array[i].name}</a>\n</div>\n<div class="bio">${array[i].bio}</div>\n</div>`)
}
...
const randomNum = Math.floor(Math.random() * array.length)
    while (randomizedArray.length < array.length) {
      const randomNum = Math.floor(Math.random() * array.length)
      if (!randomizedArray.includes(array[randomNum])) {
...
const artistsBornIn1900s = artists.filter(object=> {
  let yearsArray = object.years.split('-');
  return parseInt(yearsArray[0]) > 1900;
});